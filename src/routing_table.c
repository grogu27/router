...
//                 // ОТЛАДКА
//     printf("DEBUG: network_str=%s, nexthop_str=%s\n", network_str, nexthop_str);
...
//                 // ОТЛАДКА
//     struct in_addr test;
//     test.s_addr = route.nexthop;
//     printf("DEBUG: nexthop converted = %s\n", inet_ntoa(test));
...
//                 printf("DEBUG PRINT: route[%d] network=%u, nexthop=%u", 
//                     i, route_table[i].network, route_table[i].nexthop);
...
// Преобразование строки IP в uint32_t (сетевой порядок) с использованием inet_pton
uint32_t ip_to_uint32(const char *ip_str) {
    struct in_addr addr;
...
// Преобразование uint32_t в строку IP (сетевой порядок) с использованием inet_ntop
...
int resolve_mac(const char *interface_name, uint32_t ip, unsigned char *mac) {
    ...
    uint32_to_ip(ip, ip_str, sizeof(ip_str));
...
int save_route_to_file(const char *dir_path, route_entry_t *route) {
    ...
    uint32_to_ip(route->network, ip_str, sizeof(ip_str));
    fprintf(f, "%s\n", ip_str);
    
    uint32_to_ip(route->mask, ip_str, sizeof(ip_str));
    fprintf(f, "%s\n", ip_str);
    
    uint32_to_ip(route->nexthop, ip_str, sizeof(ip_str));
    fprintf(f, "%s\n", ip_str);
...
void print_routing_table() {
    ...
    for (int i = 0; i < route_count; i++) {
        ...
        uint32_to_ip(route_table[i].network, network_str, sizeof(network_str));
        uint32_to_ip(route_table[i].mask, mask_str, sizeof(mask_str));
        uint32_to_ip(route_table[i].nexthop, nexthop_str, sizeof(nexthop_str));
